"""
Resource Allocation Index for Friend Recommendation
...
def compute_resource_allocation_score(graph: nx.Graph, u: int, v: int) -> float:
	"""Compute Resource Allocation score between two nodes.
...
def predict_links_for_node(graph: nx.Graph, node: int, k: int = 10) -> List[Tuple[int, float]]:
	...
	for candidate in graph.nodes():
		if candidate != node and candidate not in neighbors:
			score = compute_resource_allocation_score(graph, node, candidate)
			if score > 0:
...
def predict_all_links(graph: nx.Graph, existing_edges: Set[Tuple[int, int]]) -> List[Tuple[int, int, float]]:
	"""Predict RA scores for all possible non-existing edges in the graph."""
	...
	for i, u in enumerate(nodes):
		for v in nodes[i + 1 :]:
			if (u, v) not in existing_edges and (v, u) not in existing_edges:
				score = compute_resource_allocation_score(graph, u, v)
				if score > 0:
...
def evaluate_algorithm(train_graph: nx.Graph, test_edges: List[Tuple[int, int]], k: int = 10) -> Dict[str, float]:
	"""Evaluate Resource Allocation on a train/test split.

	Returns precision, recall, F1, runtime and basic counts.
	"""
...
def calculate_metrics_at_multiple_k(
	...
	"""Evaluate RA at multiple k values."""
...
def demo() -> None:
	"""Demo for Resource Allocation Index using the Facebook ego dataset."""

	print("=" * 70)
	print("Resource Allocation Index Algorithm - Demo")
	print("=" * 70)
	...
	print(f"\n{'='*70}")
	print(f"Top 5 friend recommendations for user {node} (RA):")
	print(f"{'='*70}")
