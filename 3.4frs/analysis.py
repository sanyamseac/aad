...
# Import the 3 friend recommendation functions from files in this folder
from aa import compute_adamic_adar_score, predict_links_for_node as aa_predict
from cm import compute_common_neighbors_score, predict_links_for_node as cm_predict
from jc import compute_jaccard_coefficient, predict_links_for_node as jc_predict

# Import 'graph.py' from the parent 'aad' directory
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
from graph import create_complete_graph

def evaluate_algorithm_detailed(train_graph, test_edges, predict_func, k_values=[5, 10, 20, 50], sample_size=1000):
    """
...
def compare_algorithms_on_node(G, node, k=5):
    """
    Compare all three algorithms' recommendations for a specific node.
...
def main():
    ...
    print("\n" + "="*70)
    print("TOP RECOMMENDATIONS FOR SAMPLE NODES")
    print("="*70)
    ...
    for algo in ['Adamic-Adar', 'Common Neighbors', 'Jaccard Coefficient']:
    ...
    # Bar chart for each metric at k=20
    k20_data = performance_df[performance_df['k'] == 20]
    algorithms = k20_data['Algorithm'].values
    
    x_pos = np.arange(len(algorithms))
    
    # Precision
    axes[0].bar(x_pos, k20_data['Precision'], color=['#1f77b4', '#ff7f0e', '#2ca02c'])
    axes[0].set_xlabel('Algorithm')
    axes[0].set_ylabel('Precision')
    axes[0].set_title('Precision Comparison (k=20)')
    axes[0].set_xticks(x_pos)
    axes[0].set_xticklabels(algorithms, rotation=15, ha='right')
    axes[0].grid(True, alpha=0.3, axis='y')
    
    # Recall
    axes[1].bar(x_pos, k20_data['Recall'], color=['#1f77b4', '#ff7f0e', '#2ca02c'])
    axes[1].set_xlabel('Algorithm')
    axes[1].set_ylabel('Recall')
    axes[1].set_title('Recall Comparison (k=20)')
    axes[1].set_xticks(x_pos)
    axes[1].set_xticklabels(algorithms, rotation=15, ha='right')
    axes[1].grid(True, alpha=0.3, axis='y')
    
    # F1-Score
    axes[2].bar(x_pos, k20_data['F1-Score'], color=['#1f77b4', '#ff7f0e', '#2ca02c'])
    axes[2].set_xlabel('Algorithm')
    axes[2].set_ylabel('F1-Score')
    axes[2].set_title('F1-Score Comparison (k=20)')
    ...
    print("\nOverall Best Performance (at k=20):")
    k20_data_sorted = k20_data.sort_values('F1-Score', ascending=False)
    print("\nBy F1-Score:")
    print(k20_data_sorted[['Algorithm', 'Precision', 'Recall', 'F1-Score']].to_string(index=False))
    
    print("\nBy True Positives:")
    k20_data_sorted_tp = k20_data.sort_values('True Positives', ascending=False)
