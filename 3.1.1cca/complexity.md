...
| Algorithm | Time Complexity | Space Complexity | Constant Factors |
| :--- | :--- | :--- | :--- |
| **BFS** | $O(V + E)$ | $O(V)$ | Moderate (Queue overhead) |
| **DFS** | $O(V + E)$ | $O(V)$ | Low (Simple list stack) |
| **UFA (Rank)** | $O(E \cdot \alpha(V))$ | $O(V)$ | Higher (Dict lookups) |
| **UFA (Size)** | $O(E \cdot \alpha(V))$ | $O(V)$ | Higher (Dict lookups) |

*Note: In dense social graphs where $E \gg V$, the $E$ term dominates. All four algorithms effectively scale linearly with the number of edges.*